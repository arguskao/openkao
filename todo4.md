...
8. 一維條碼。
9. 左右兩個 QRCode。
10. 僅有買方統編的 B2B 發票接續列印銷貨明細單；無買方統編的 B2C 發票在 QRCode 後結束。
...
- [x] B2C 不顯示 `格式:25`，且不列印銷貨明細單、商品、銷售額與稅額。
- [x] B2B 銷貨明細單顯示買受人統編、營業人統編、公司名稱與交易時間。
- [x] B2B 每筆商品的金額後附加課稅別 `TX`，例如 `2,400TX`。
...
- [x] B2C 不列印交易明細；B2B 才列印品名、數量、單價、`金額TX`、銷售額、稅額與總計。
...
- B2C 發票在雙 QRCode 後結束，不列印 B2B 專用的銷貨明細單。
- B2B 每筆明細金額後顯示 `TX`。
